%Diffusion-Directed Diffusion Demonstration of JDD Method
%Rebecca Menssen
...
%this appears to be the most finicky of combination models, improvements
%can be made. 
...
Dalpha=1; %micro meters^2/s
alpha=0.6; %micro meters/s
...
%Simulate Diffusion:
[x1]=Diffusion1D(D,points,N1,dt,seed1);
[x2]=AnomalousDiffusion1D(Dalpha,alpha,points,N2,dt,tau,seed2);
x=horzcat(x1,x2);
...
%Nb=round(1+log2(N)+log2(1+abs(skewness(jd))/sigma));
...
%Plot the predicted JDD on top of it
predictedJDD=fd*N*dr/((pi*D*tau)^(1/2)).*exp(-ri.^2/(4*D*tau))+(1-fd)*(2*N*dr/((Dalpha)^(1/2)*2).*abs(integral(fun,min,-1*min,'ArrayValued',true,'AbsTol',2e-5)));
hold on
plot(ri,predictedJDD,'k','LineWidth',1.5)

xlabel('Jump Distance')
ylabel('Count')
title('Diffusion-Anomalous Combination Jump Distance Distribution in 1D')
...
legend('Jump Distance Distribution',['Predicted Doubld Diffusion Fit,D=',num2str(D),', D_\alpha=',num2str(Dalpha),', \alpha=',num2str(alpha),', fd=',num2str(fd)],...
...
parfor i=1:numboot
    ...
    %Nb=round(1+log2(N)+log2((1+skewness(jdB)/(sqrt((6*(N-2))/((N+1)*(N+3))))))); %Doanes Rule
