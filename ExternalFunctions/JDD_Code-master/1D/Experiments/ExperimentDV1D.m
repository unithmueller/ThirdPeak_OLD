...
[x2]=DirectedMotion1D(V,Dv,points,N2,dt,seed2);
x=horzcat(x1,x2);
...
y = ri*V/(2*Dv);
predictedJDD=fd*N*dr/((pi*D*tau)^(1/2)).*exp(-ri.^2/(4*D*tau))+(1-fd)*(N*dr/((4*pi*Dv*tau)^(1/2)).*exp(z+y)+N*dr/((4*pi*Dv*tau)^(1/2)).*exp(z-y));
hold on
plot(ri,predictedJDD,'k','LineWidth',1.5)

xlabel('Jump Distance')
ylabel('Count')
title('Diffusion-Directed Motion Jump Distance Distribution in 1D')
...
legend('Jump Distance Distribution',['Predicted DV Fit,D=',num2str(D),', V=',num2str(V),', Dv=',num2str(Dv) ', fd=',num2str(fd)],...
    ...
    ['Fit DV,D=',num2str(param.Ddv),', V=',num2str(param.Vdv),', Dv=',num2str(param.Dvdv) ', fd=',num2str(param.fdDV)],...
