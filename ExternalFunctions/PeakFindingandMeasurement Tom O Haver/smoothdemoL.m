function smoothdemoL
% Function that demonstrates smoothing optimization for the measurement of
% the height of a noisy Lorentzian peak. Compares sliding-average,
...
for point=1:maxpoint,
    ...
    y=ones(size(x))./(1+((x-5)./(0.5)).^2);
    if point==1,
        subplot(2,2,1)
        plot(x,y)
        title('Original Lorentzian, half width = 322 points')
        xlabel('x')
        ylabel('y')
        axis([0 10 0 1])
    end
