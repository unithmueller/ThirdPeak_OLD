% Test script for peakfit.m version 8.4 or later. Requires peakfit.m,
% ExpBroaden.m, gaussian.m, lorewtzian.m, humps.m, DataMatrix3.mat to be
% in the path. Version 6.2, August 2019
warning off all
disp('-------------------------------------------------------------------')
...
echo off
if FitError(1)>15,beep;disp('WARNING: Fitting error is greater that 15%; may indicate a malfunction.');end
drawnow;echo on
%---------------------------------------------------------------------
...
echo off; if FitError(1)>15,beep;disp('WARNING: WARNING: Fitting error is greater that 15%; may indicate a malfunction.');end
drawnow
disp(' ')
echo on
%---------------------------------------------------------------------
...
echo off; if FitError(1)>15,beep;beep;disp('WARNING: Fitting error is greater that 15%; may indicate a malfunction.');end
drawnow
disp(' ')
echo on
%--------------------------------------------------------------------- 
...
echo off; if FitError(1)>15,beep;disp('WARNING: Fitting error is greater that 15%; may indicate a malfunction.');end
drawnow
disp(' ')
echo on
  %---------------------------------------------------------------------  
...
echo on
%---------------------------------------------------------------------
% Example 21:  (Version 3.9 or later) Fitting a portion of the humps
% function with two Voigt profiles
[FitResults,FitError]=peakfit(humps(0:.01:2),71,110,2,20,1.7,1,[31 4.7 90 8.8],0)
echo off; if FitError(1)>15,beep;disp('WARNING: Fitting error is greater that 15%; may indicate a malfunction.');end
drawnow
disp(' ')
echo on
...
% of each peak.
x=[0:.005:.6];y=humps(x);[FitResults,FitError]=peakfit([x' y'],60,120,1,30)
echo off; if FitError(1)>15,beep;disp('WARNING: Fitting error is greater that 15%; may indicate a malfunction.');end
drawnow
disp(' ')
echo 
...
echo 
%---------------------------------------------------------------------
% Example 37: Variable time constant exponentially-broadened Lorentzian
...
echo 
%---------------------------------------------------------------------
% Example 38: 3-parameter logistic (Gompertz), shape 43. (Version 7.9 and
...
echo 
%---------------------------------------------------------------------
% Example 39: Shape 46, 'quadslope' (version 8 and above). Two overlapping
...
x=7:.05:13;
y=x.^2/50+exp(-(x-9).^2)+exp(-(x-11).^2)+.02.*randn(size(x));
