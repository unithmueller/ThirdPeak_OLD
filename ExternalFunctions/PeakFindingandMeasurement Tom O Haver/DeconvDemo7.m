...
% closely-spaced underlying lorentzian peaks. Requires lorentzian,triangle.
% and fastsmooth functions .

increment=.001;
position=10;
width=.5; % Width of the original lorentzian peak.
cw=1.5; % cw = Convolution width of the physical convolution process (unknown)
dw=1.5; % dw = Deconvolution width (estimated) must equal cw for perfect results
...
% Convolute underlying peaks with triangle convolution function
yc=ifft(fft(y).*fft(c))./sum(c);  % Create triangle convoluted rectangular function
