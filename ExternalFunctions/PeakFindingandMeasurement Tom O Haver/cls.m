function heights=cls(xx,yy,NumPeaks,peakshape,Positions,Widths,extra)
...
% and BiLorentzian shapes.  Cls.m returns a vector of measured peak heights
% for each peak. clsdemo.m is a demonstration script that creates some
% noisy model data, fits it with cls.m, then computes the accuracy of the
% measured heights. 
%
mm=modelmatrix(xx,NumPeaks,peakshape,Positions,Widths,extra);
heights=yy/mm;
size(modelmatrix(xx,NumPeaks,peakshape,Positions,Widths,extra));
plot(xx,yy,'r.',xx,heights*mm,'b')
