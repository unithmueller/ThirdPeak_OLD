% Comparison of deconvolution to power transform method (Lorentzian peaks)
format compact
format short g
xx=0:.1:128;
power=8;
SmoothWidth=5; % Smooth width for convoluted signal (e.g. 3 - 12)
% Underlying signal with true peak shape (lorentzian),heights, positions, and widths.
uyy=1.2.*lorentzian(xx,25,10)+lorentzian(xx,55,10);   
...
subplot(2,2,1);plot(xx,uyy);title('Unobserved underlying Lorentzian peaks, uyy');
...
subplot(2,2,4);plot(xx,yy./max(yy),xx,yyp./max(yyp));title(['Raising the signal to a power, yyp. Power= ' num2str(power)] )

% Use the findpeaksL function to measure peak position, heights, widths
disp('Peaks of the Underlying Lorentzian signal, uyy')
disp('          Peak#     Position     Height      Width          Area')
Puyy=findpeaksL(xx,uyy,6e-005,0.6,10,10,3);
disp(Puyy)
SeparationToWidthRatio=(Puyy(2,2)-Puyy(1,2))./Puyy(1,4)
disp(' ')
disp('Peaks of observed broadened and noisy signal, yy')
disp('          Peak#     Position     Height      Width          Area')
Pyy=findpeaksL(xx,yy,6e-005,0.2,10,10,3);
disp(Pyy)
SeparationToWidthRatio=(Pyy(2,2)-Pyy(1,2))./Pyy(1,4)
disp(' ')
disp('Peaks of the smoothed deconvoluted recovered signal, yydc')
disp('          Peak#     Position     Height      Width          Area')
Pyydc=findpeaksL(xx,yydc,6e-005,0.6,50,80,3);
disp(Pyydc)
...
disp('          Peak#     Position     Height      Width          Area')
Pyy2=findpeaksL(xx,yyp,1e-005,0.01,10,10,3);
disp(Pyy2)
