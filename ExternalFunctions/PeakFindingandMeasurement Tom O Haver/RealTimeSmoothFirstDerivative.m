% Here's an example of real-time first differentiation that you can run on
% your computer in Matlab or Octave without any additional hardware.  Lines
% 9-28 generate a simulated signal and a smoothing vector. Lines 28-49
...
clf
clear
maxx=1000; % Sets the maxumum number of data points
...
% >>Uncomment one of the following 3 lines to define the smooth shape.<<
...
RawData=gaussian(1:maxx,maxx/2.5,maxx/5)+.01.*randn(size(y)); % Generate sumulated data
grid
tic
for n=1:maxx
    ...
    if n==1 % Comment out or remove this 'if' loop to disable plotting
        plot(1,sy(1)) % Plot the first simulated data point
        xlabel(['X      SmoothWidth= ' num2str(SmoothWidth)]);ylabel('Y') % Label axes
        title('Real Time Smooth First Derivative Demo.  Black=original   Red=smoothed derivative')
    else
        plot([n-1 n],[y(n-1) y(n)],'k')
        dy(n)=(maxx/10).*((sy(n)-sy(n-1)));
        dy(1:SmoothWidth+2)=0; % Zeros partially smoothed derivative points
