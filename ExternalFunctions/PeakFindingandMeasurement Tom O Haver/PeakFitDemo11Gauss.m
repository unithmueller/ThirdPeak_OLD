...
for k=1:length(pos),
  A(k,:)=gaussian(x,pos(k),wid(k));  % Gaussian function
...
plot(x,y)
drawnow
disp('Test signal with 11 overlapping Gaussian peaks ')
...
disp(' ')
disp('This results in a large fit error and poor peak parameter accuracy.')
disp(' ')
...
disp('>> [FitResults,FitError]=peakfit([x;y],0,0,11,1,0,1,GoodStart,0)')
ColumnLabels
GoodStart=[400 20 430 40 500 50 600 40 700 20 730 20 800 30 850 40 900 60 1000 100 1200 200];
[FitResults,FitError]=peakfit([x;y],0,0,11,1,0,1,GoodStart,0)
disp(' ')
