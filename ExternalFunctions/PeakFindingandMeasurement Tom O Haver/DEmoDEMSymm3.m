...
separation=400; % Separation between peaks
...
L1=98;  % Adjust this after L2 is optimized

Delta=.5; % Produces +/-1% brackets

% Construction of exponentially modified Gaussian (EMG)
tau1=1/lambda1;
EMG=A.*expgaussian2(t,mu,s,tau1)+0.5.*A.*expgaussian2(t,mu+separation,s,tau1)+Noise.*randn(size(t));
...
% Call DEMG1stDerivativeManualSymm function
rG=DEMG1stDerivativeManualSymm(t,DEMG,L1,L2,Delta);
...
[FitResults,GOF]=peakfit([t;rG],0,0,2,1,0,10,[mu 300 mu+separation 300]);
