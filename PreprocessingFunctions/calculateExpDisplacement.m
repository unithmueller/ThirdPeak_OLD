function expdisplacementvaluestring = calculateExpDisplacement(tracks, tracklengths)
    ...
    if size(tracks,2) >= 14 %data from swift analysis
        ...
        tmpdat = [tracks{1,2}, tracklengths{1,2}];
        tmpdat = tmpdat(tmpdat(:,1)~=0 | tmpdat(:,1)~=0,:);
        avgWghtMJD = sum(tmpdat(:,1).*tmpdat(:,2))/sum(tmpdat(:,2));
    end
